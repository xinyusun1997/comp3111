	...
	public List<Item> scrape(String keyword) {
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				//haoqi
				HtmlElement postTime = (HtmlElement) htmlItem.getFirstByXPath(".//p[@class='result-info']/time");
						
				Item item = new Item();
				item.setTitle(itemAnchor.asText());
				//item.setUrl(DEFAULT_URL + itemAnchor.getHrefAttribute());   wrong!!!
				item.setUrl(itemAnchor.getHrefAttribute());
				item.setPrice(new Double(itemPrice.replace("$", "")));
				
				//haoqi
				String mystr = postTime.getAttribute("datetime")+" "+postTime.getAttribute("title");
				String[] timeArr = mystr.split(" ");
				item.setTime(timeArr);
				//haoran
				item.setTimeString(mystr); 
